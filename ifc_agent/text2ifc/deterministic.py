...
class DeterministicArchitect:
    ...
    def design_spatial(self, requirements: dict, *,
        ...
        # Structural system: default to frame; use core_tube for tall buildings
        struct_kind = "frame"
        if n_storeys >= 5:
            struct_kind = "core_tube"
        elif n_storeys >= 3 and total_columns > 0:
            struct_kind = "mixed"
        elif "shear" in requirements.get("structural_system", "").lower():
            struct_kind = "shear_wall"
        elif "core" in requirements.get("structural_system", "").lower():
            struct_kind = "core_tube"

        structural_system = StructuralSystem(
            kind=struct_kind,
            grid_spacing_x_mm=6000.0,
            grid_spacing_y_mm=6000.0,
        ...
        # Slabs: every inhabited storey needs at least one floor slab.
        # If total_slabs >= n_inhab, give each inhabited storey 1 slab
        # and pile the remainder on the ground floor (podium / transfer slab).
        # If total_slabs < n_inhab, fall back to proportional split.
        if total_slabs >= n_inhab:
            slab_per = [1 if h else 0 for h in inhabited]
            remaining = total_slabs - n_inhab
            if remaining > 0 and inhabited[0]:
                slab_per[0] += remaining
        else:
            slab_per = _split_among(total_slabs, inhabited)

        roof_per = [0] * n_storeys
        if n_roofs > 0:
            roof_per[-1] = n_roofs

        # Vertical shafts: stair + optional elevator for multi-storey
        shafts: list[VerticalShaft] = []
        if n_storeys >= 2:
            stair_shaft = VerticalShaft(
                id="stair_main",
                kind="stair",
                storey_ids=[f"s{i + 1}" for i in range(n_storeys)],
                shaft_mm=(2400.0, 4200.0),
                wall_thickness_mm=200.0,
                material="Concrete",
            )
            shafts.append(stair_shaft)
            if n_storeys >= 4:
                elevator = VerticalShaft(
                    id="elevator_main",
                    kind="elevator",
                    storey_ids=[f"s{i + 1}" for i in range(n_storeys - 1)],
                    shaft_mm=(2000.0, 2400.0),
                    wall_thickness_mm=200.0,
                    material="Concrete",
                )
                shafts.append(elevator)
        ...
        for i in range(n_storeys):
            ...
            # Add shaft room references
            shaft_ids = [s.id for s in shafts if f"s{i + 1}" in s.storey_ids]
            # For stair shaft, add a stairwell room
            if inhabited[i] and "stair_main" in shaft_ids:
                ...
                # Ensure corridor connects to stairwell
                for r in rooms:
                    if r.id == "corridor":
                        r.adjacent_to.append("stairwell")
                        r.opening_to.append("stairwell")
