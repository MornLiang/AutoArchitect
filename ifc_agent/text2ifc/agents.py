...
def _autorepair_spatial(sg: dict, requirements: dict | None = None) -> dict:
    ...
    for idx, s in enumerate(storeys):
        ...
        s.setdefault("structural_system_override", None)
        if rooms:
    ...
    # NEW: shafts and structural system
    sg.setdefault("shafts", [])
    ss = sg.setdefault("structural_system", {})
    if not isinstance(ss, dict):
        ss = {}
        sg["structural_system"] = ss
    ss.setdefault("kind", "frame")
    ss.setdefault("grid_spacing_x_mm", 6000)
    ss.setdefault("grid_spacing_y_mm", 6000)
    ss.setdefault("core_position", "center")
